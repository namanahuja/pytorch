...
class PostGradBatchLinearFusion(BatchFusion):
    ...
    def fuse(self, graph: torch.fx.GraphModule, subset: List[torch.fx.Node]):
        ...
        for node in subset:
            ...
            batch_inputs_meta.append(input.meta)  # type: ignore[possibly-undefined]
            batch_weights_meta.append(weight.meta)  # type: ignore[possibly-undefined]
            if bias is not None:  # type: ignore[possibly-undefined]
                batch_biases_meta.append(bias.meta)  # type: ignore[possibly-undefined]
            else:
        ...
        with graph.inserting_before(subset[-1]):
            ...
            fused_inputs_meta_val = torch.stack([input["val"] for input in batch_inputs_meta])
            fused_weights_meta_val = torch.stack([weight["val"] for weight in batch_weights_meta])
            fused_bmm = graph.call_function(
                aten.bmm,
                args=(fused_inputs, fused_weights),
            )
            fused_bmm.meta["val"] = aten.bmm(fused_inputs_meta_val, fused_weights_meta_val)
        for i, original_mm in enumerate(batch_nodes):
            ...
            with graph.inserting_after(fused_bmm):
                ...
                if batch_biases[i]:
                    ...
                    if self.graph_search_options.get("shape_broadcast_batch_linear", False):
                        ...
                        broadcast_bias = graph.call_function(
                            aten.broadcast_to.default, args=(batch_biases[i],), kwargs={"size": broadcast_shape}
                        )
                        ...
                        new_bias_add.meta["val"] = aten.add.Tensor(broadcast_bias.meta["val"], new_mm.meta["val"])
                    else:
                        new_bias_add = graph.call_function(
                            aten.add, args=((batch_biases[i], new_mm))
                        )
                        new_bias_add.meta["val"] = aten.add.Tensor(batch_biases_meta[i]["val"], new_mm.meta["val"])
...
class BatchPointwiseMathOpsPostGradFusion(BatchPointwiseOpsFusionFactory):
        ...
        if CallFunctionVarArgs(self.op).match(
            ...
            if self.graph_search_options.get("fuse_nodes_with_same_parent", False):
                ...
                if input.target == aten.select or other.target == aten.select:
                    # pyre-fixme[16]
                    parent = input.args[0] if input.target == aten.select else other.args[0]
                else:
                    parent = ''
            else:
                parent = ''
    ...
    def fuse(self, graph: torch.fx.GraphModule, subset: List[torch.fx.Node]):
        ...
        for node in subset:
            ...
            batch_inputs_meta.append(input.meta)  # type: ignore[possibly-undefined]
            batch_others_meta.append(other.meta)  # type: ignore[possibly-undefined]

        with graph.inserting_before(subset[0]):
            stack_inputs = decompose_stack(graph, batch_inputs)
            stack_others = decompose_stack(graph, batch_others)
            stack_inputs_meta = torch.stack([input["val"] for input in batch_inputs_meta])
            stack_others_meta = torch.stack([other["val"] for other in batch_others_meta])
