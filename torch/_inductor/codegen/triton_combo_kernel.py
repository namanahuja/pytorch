    ...
    class SequentialDispatch:
        ...
        def _calculate_xblocks(cls, kernel, code):
            ...
            for i in range(len(x_numels_list)):
                xnumels = x_numels_list[i] if x_numels_list[i] > 0 else kernel.min_x_blocks_list[i]
                xblock_str = f"tl.cdiv({xnumels}, XBLOCK)" if x_numels_list[i] > 0 else f"{xnumels}"
                if i == 0:
